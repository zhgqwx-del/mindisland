...
- **Bridge scripts** (`hooks/`): Claude Code bash hook + OpenCode JS plugin, forward events via Unix socket
- **Docs** (`docs/`): Architecture, integration guide, lessons learned, roadmap

Data flow: `Agent → Bridge (hook/plugin) → Unix socket → claude.rs → SessionManager → Frontend`
...
- Agent adapters live in `src-tauri/src/agents/`. All agents share the same socket and event format.
- Hooks/plugins **fail open** — if MindIsland is not running, agents continue unaffected.
...
- **Mutex discipline**: release `sessions` mutex BEFORE doing emit, file I/O, or window operations. Use scoped blocks.
- **Phase protection**: `WaitingForApproval`/`WaitingForAnswer` phases must not be overwritten by `Running` (ActivityUpdated).

## Current Status

- ✅ Claude Code: full integration (15 hook events, permission approval, session discovery, auto-install, panel auto-popup)
- ✅ OpenCode: JS plugin integration (event forwarding, permission/question reply, auto-install)
